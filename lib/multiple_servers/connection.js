...
Streamy.Connection = function(connection) {
  ...
  self._deleteHandlers = function _deleteHandlers() {
    Object.keys(_handlers).forEach(function(func) {
  ...
  _connection._stream.on('disconnect', function onClose() {
    ...
    if(_connection._stream.status().connected) {
      _disconnectHandlers.forEach(function forEachDisconnectHandler(cb) {
        cb.call(self);
      });
    }
  });
  ...
  _connection._stream.on('message', function onMessage(data) {
    ...
    if(msg === 'connected') {
      // Call each handlers
      _connectHandlers.forEach(function forEachConnectHandler(cb) {
        cb.call(self);
      });
    } else if(msg) {
      // Else, call the appropriate handler
      const f = _handlers[msg] || function() {};
